...
public class ESSimpleExpression {
	...
	protected ESSimpleExpression(String fieldName, Collection<Object> values,ESCriterion.Operator operator) {
		this.fieldName = fieldName;
		this.values = values;
	...
	public QueryBuilder toBuilder() {
		...
		case RANGE:
			...
			break;
		case GT:
			qb = QueryBuilders.rangeQuery(fieldName).gt(value).includeLower(false);
			break;
		case LT:
			qb = QueryBuilders.rangeQuery(fieldName).lt(value).includeUpper(false);
			break;
		case FUZZY:
